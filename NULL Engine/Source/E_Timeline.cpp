...
bool E_Timeline::Draw(ImGuiIO& io)
{
	...
	ImGui::TimelineEventEx("Take001", values, false);
	...
	current_time += Time::Game::GetDT() * 24;
...
bool ImGui::TimelineEvent(const char* str_id, float* values)
{
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		pos.x += (win->Size.x * values[i]) / (max_timeline_value + TIMELINE_RADII);
		pos.y += TIMELINE_RADII;

		SetCursorScreenPos(pos - ImVec2(TIMELINE_RADII, TIMELINE_RADII));
		PushID(i);
		InvisibleButton(str_id, ImVec2(2 * TIMELINE_RADII, 2 * TIMELINE_RADII));
		...
		PopID();
		win->DrawList->AddCircleFilled(pos, TIMELINE_RADII, (IsItemActive() || IsItemHovered()) ? active_color : inactive_color);
	}

	ImVec2 start	= cursor_pos;
	start.x			+= (win->Size.x * values[0]) / (max_timeline_value + 2 * TIMELINE_RADII);
	start.y			+= TIMELINE_RADII * 0.5f;
	ImVec2 end		= start + ImVec2(win->Size.x * ((values[1] - values[0]) / (max_timeline_value - 2 * TIMELINE_RADII)), TIMELINE_RADII);
...
void ImGui::EndTimeline()
{
	...
	{
		ImVec2 a	= GetWindowContentRegionMin() + win->Pos + ImVec2(TIMELINE_RADII, 0);
